...
1. **Document Processing**: Extracts structured data from patent PDFs including front matter, claims, descriptions, and tables
...
3. **Intelligent Chunking**: Creates semantic chunks that preserve document structure and table headers
4. **Hybrid Search**: Combines lexical (BM25) + semantic (vector) search with cross-encoder reranking
5. **Context-Aware Retrieval**: Returns relevant chunks with proper context for Q&A
...
- **Contextual Results**: Each result includes patent context and section type
...
- Visit `http://127.0.0.1:8000/` in the browser to load the demo interface. The UI displays patents with LLM-generated summaries and supports natural language queries.
- Manual API calls are available via `http`/`curl`:
  ...
  ```
- `POST /search/chunks`: accepts `{ "query": str, "patent_id": Optional[str], "top_k": int }` and returns reranked chunk snippets (lexical + semantic fusion with cross-encoder).
